...
if(isset($_GET['action']) && $_GET['action'] == "affichage")
{
	...
	$contenu .= '<h2 class="text-center"> Affichage des produits </h2>';
	$contenu .= 'Nombre de produit(s) dans la boutique : ' . $resultat->num_rows;
	$contenu .= '<table border="1" cellpadding="5" class="table table-striped table-bordered"><tr>';
	
	while($colonne = $resultat->fetch_field())
	{    
		$contenu .= '<th class"text-primary">' . $colonne->name . '</th>';
	}
	$contenu .= '<th class"text-primary">Modification</th>';
	$contenu .= '<th class"text-primary">Supression</th>';
	$contenu .= '</tr>';
	...
	while ($ligne = $resultat->fetch_assoc())
	{
		...
		$contenu .= '<td><a href="?action=suppression&id_produit=' . $ligne['id_produit'] .'" OnClick="return(confirm(\'En étes vous certain ?\'));"><img src="../inc/img/delete.png" /></a></td>';
		$contenu .= '</tr>';
	}
...
if(isset($_GET['action']) && ($_GET['action'] == 'ajout' || $_GET['action'] == 'modification'))
{
	...
	<form method="post" enctype="multipart/form-data" action="" class="w-75 mx-auto shadow rounded p-5">
	<h1 class="text-center text-primary"> Formulaire Produits </h1>
		...
		<label class="form-label" for="reference">reference</label><br />
		<input class="form-control" type="text" id="reference" name="reference" placeholder="la réfèrence de produit" value="'; if(isset($produit_actuel['reference'])) echo $produit_actuel['reference']; echo '" /><br /><br />

		<label class="form-label" for="categorie">categorie</label><br />
		<input class="form-control" type="text" id="categorie" name="categorie" placeholder="la categorie de produit" value="'; if(isset($produit_actuel['categorie'])) echo $produit_actuel['categorie']; echo '"  /><br /><br />

		<label class="form-label" for="titre">titre</label><br />
		<input class="form-control" type="text" id="titre" name="titre" placeholder="le titre du produit" value="'; if(isset($produit_actuel['titre'])) echo $produit_actuel['titre']; echo '"  /> <br /><br />

		<label class="form-label" for="description">description</label><br />
		<textarea class="form-control" name="description" id="description" placeholder="la description du produit">'; if(isset($produit_actuel['description'])) echo $produit_actuel['description']; echo '</textarea><br /><br />
		
		<label class="form-label" for="couleur">couleur</label><br />
		<input class="form-control" type="text" id="couleur" name="couleur" placeholder="la couleur du produit"  value="'; if(isset($produit_actuel['couleur'])) echo $produit_actuel['couleur']; echo '" /> <br /><br />
		...
		<label class="form-label" for="public">public</label><br />
		...
		echo '
		<label class="form-label" for="prix">prix</label><br />
		<input class="form-control" type="text" id="prix" name="prix" placeholder="le prix du produit"  value="'; if(isset($produit_actuel['prix'])) echo $produit_actuel['prix']; echo '" /><br /><br />
		
		<label class="form-label" for="stock">stock</label><br />
		<input class="form-control" type="text" id="stock" name="stock" placeholder="le stock du produit"  value="'; if(isset($produit_actuel['stock'])) echo $produit_actuel['stock']; echo '" /><br /><br />
		
		<input class="btn btn-primary w-50 mx-auto d-block" type="submit" value="'; echo ucfirst($_GET['action']) . ' du produit"/>
	</form>';
}
